...
if uploaded_file:
    ...
    if missing_cols:
        ...
        st.info("💡 Pastikan file mentahan Anda sudah ada kolom 'Tsh' dan 'Area'.")
    else:
        # Cleansing Isi Data
        ...
        if df.empty:
            ...
            # 3. Proses Pembuatan Pivot Table (Format MultiIndex Asli)
            ...
            # Tampilkan sekilas di Web Streamlit (Tampilan aman anti-crash)
            st.info("💡 Tampilan di web memang disingkat oleh sistem. Silakan klik Unduh untuk melihat format aslinya di Excel.")
            st.dataframe(pivot_df, use_container_width=True)
            
            # 4. Export ke File Excel (.xlsx) dengan Format Custom (Persis Gambar SPR JABO)
            buffer = io.BytesIO()
            ...
            # Hapus baris ke-4 (baris jeda kosong bawaan Pandas) agar data langsung rapat ke atas
            ws.delete_rows(4)
            
            # Tambahkan tombol Filter (Dropdown) di baris ke-3 persis seperti gambar
